...
public class PayrollController {
    ...
    @GetMapping("")
    public String viewPayrollPeriods(Model model, HttpSession session) {
    ...
    public String showNewPayrollPeriodForm(Model model, HttpSession session) {
        if (session.getAttribute("loggedInUser") == null) {
            return "redirect:/login";
        }
    ...
    public String processPayroll(@PathVariable("id") Long id, HttpSession session) {
        if (session.getAttribute("loggedInUser") == null) {
            return "redirect:/login";
        }
    ...
    public String viewPayrollRecords(@PathVariable("periodId") Long periodId, Model model, HttpSession session) {
        if (session.getAttribute("loggedInUser") == null) {
            return "redirect:/login";
        }
    ...
    public String showAdjustPayrollRecordForm(@PathVariable("id") Long id, Model model, HttpSession session) {
        if (session.getAttribute("loggedInUser") == null) {
            return "redirect:/login";
        }
